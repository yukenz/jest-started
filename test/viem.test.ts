import {describe, test} from '@jest/globals';
import {createTestClient, erc20Abi, formatUnits, http, keccak256, publicActions, stringToHex, walletActions} from 'viem'
import {foundry} from 'viem/chains'
import {privateKeyToAccount} from "viem/accounts";
import {eip712abi} from "../src/eip712abi.ts";
import {domain, types} from "../src/eip712-walle.ts";

describe.skip('viem test', () => {

    test.skip('test client', async () => {

        const client = createTestClient({
            chain: foundry,
            mode: 'anvil',
            transport: http(),
        })
            .extend(publicActions)
            .extend(walletActions)

        const blockNumber = await client.getBlockNumber() // Public Action
        const mine = await client.mine({blocks: 1}) // Test Action

    });

    test('test Card Self Service', async () => {

        const testClient = createTestClient({
            chain: foundry,
            mode: 'anvil',
            transport: http(),
        })
            .extend(publicActions)
            .extend(walletActions)

        const account = privateKeyToAccount('0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80')

        const message = {
            operation: 0,
            hashCard: "0xa86fd2fba383be6bb4b450c9001ea7444f651c42333174e2935b62443c182c38",
            hashPin: "0x387a8233c96e1fc0ad5e284353276177af2186e7afa85296f106336e376669f7",
        } satisfies {
            operation: number;
            hashCard: `0x${string}`;
            hashPin: `0x${string}`;
        };

        const signature = await account.signTypedData({
            domain,
            types,
            primaryType: 'CardSelfService',
            message: message,
        })
        console.log({signature})

        const recoveredAddress = await testClient.readContract({
            address: domain.verifyingContract,
            abi: eip712abi,
            functionName: 'getSignerCardSelfService',
            args: [message.operation, message.hashCard, message.hashPin, signature],
        })

        console.log({
            recoveredAddress,
            signaerAddress: account.address
        })

        expect(recoveredAddress).toEqual(account.address)
    })

    test.skip('test Card Request Payment', async () => {

        const testClient = createTestClient({
            chain: foundry,
            mode: 'anvil',
            transport: http(),
        })
            .extend(publicActions)
            .extend(walletActions)

        const account = privateKeyToAccount('0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80')


        const message = {
            hashCard: "0xa86fd2fba383be6bb4b450c9001ea7444f651c42333174e2935b62443c182c38",
            hashPin: "0x387a8233c96e1fc0ad5e284353276177af2186e7afa85296f106336e376669f7",
            merchantId: "0",
            merchantKey: "merchantKey0",
            terminalId: "0",
            terminalKey: "merchantKey0",
            paymentAmount: BigInt(10_000),
        } satisfies {
            hashCard: `0x${string}`;
            hashPin: `0x${string}`;
            merchantId: string;
            merchantKey: string;
            terminalId: string;
            terminalKey: string;
            paymentAmount: bigint;
        };
        const signature = await account.signTypedData({
            domain,
            types,
            primaryType: 'CardRequestPayment',
            message,
        })
        console.log({signature})

        const recoveredAddress = await testClient.readContract({
            address: domain.verifyingContract,
            abi: eip712abi,
            functionName: 'getSignerCardRequestPayment',
            args: [
                message.hashCard, message.hashPin,
                message.merchantId, message.merchantKey,
                message.merchantId, message.terminalKey,
                message.paymentAmount, signature],
        })

        console.log({
            recoveredAddress,
            signaerAddress: account.address
        })

        expect(recoveredAddress).toEqual(account.address)

    })


});

describe('tap2pay Card Self Service', () => {

    test('Query Card', async () => {

        /*
        * Testing Variable
        * */
        const account = privateKeyToAccount('0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80')

        /*
        * Testing Component
        * */
        const testClient = createTestClient({
            account,
            chain: foundry,
            mode: 'anvil',
            transport: http(),
        })
            .extend(publicActions)
            .extend(walletActions)


        const signature = await account.signMessage({message: "CARD_QUERY"})

        console.log({signature})

        /*
       * Validate EIP712 to Backend
       * */
        const cardListResponse = await fetch("http://localhost:8080/api/tap2pay/cards", {
            method: 'POST',
            headers: {
                'Content-Type': 'text/plain',
                'Accept': 'application/json',
            },
            body: signature
        });

        const responseText = await cardListResponse.text();

        console.log({result: cardListResponse.statusText, responseText})
    })

    test.skip('Register Card', async () => {

        /*
        * Testing Variable
        * */
        const account = privateKeyToAccount('0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80')
        const cardUUID = "94a57839-b7ef-4ff7-a1d6-54d37315a635"
        const cardPIN = "1234"


        /*
        * Testing Component
        * */
        const testClient = createTestClient({
            account,
            chain: foundry,
            mode: 'anvil',
            transport: http(),
        })
            .extend(publicActions)
            .extend(walletActions)


        /*
        * Testing Sign EIP712
        * */
        const message = {
            operation: 0,
            hashCard: keccak256(stringToHex(cardUUID)),
            hashPin: keccak256(stringToHex(cardPIN)),
        } satisfies {
            operation: number;
            hashCard: `0x${string}`;
            hashPin: `0x${string}`;
        };

        const signature = await account.signTypedData({
            domain,
            types,
            primaryType: 'CardSelfService',
            message: message,
        })
        console.log({signature})

        const recoveredAddress = await testClient.readContract({
            address: domain.verifyingContract,
            abi: eip712abi,
            functionName: 'getSignerCardSelfService',
            args: [message.operation, message.hashCard, message.hashPin, signature],
        })

        console.log({
            recoveredAddress,
            signaerAddress: account.address
        })

        expect(recoveredAddress).toEqual(account.address)

        /*
       * Validate EIP712 to Backend
       * */
        const cardRegisterResponse = await fetch("http://localhost:8080/api/v2/tap2pay/card-register", {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Accept': '*/*',
            },
            body: JSON.stringify({
                signerAddress: account.address,
                hashCard: message.hashCard,
                hashPin: message.hashPin,
                ethSignMessage: signature
            })
        });

        const responseText = await cardRegisterResponse.text();

        console.log({result: cardRegisterResponse.statusText, responseText})
    })

    test.skip('Access Card', async () => {

        /*
        * Testing Variable
        * */
        const account = privateKeyToAccount('0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80')
        const cardUUID = "94a57839-b7ef-4ff7-a1d6-54d37315a635"
        const cardPIN = "1234"


        /*
        * Testing Component
        * */
        const testClient = createTestClient({
            account,
            chain: foundry,
            mode: 'anvil',
            transport: http(),
        })
            .extend(publicActions)
            .extend(walletActions)


        /*
        * Testing Sign EIP712
        * */
        const message = {
            operation: 1,
            hashCard: keccak256(stringToHex(cardUUID)),
            hashPin: keccak256(stringToHex(cardPIN)),
        } satisfies {
            operation: number;
            hashCard: `0x${string}`;
            hashPin: `0x${string}`;
        };

        const signature = await account.signTypedData({
            domain,
            types,
            primaryType: 'CardSelfService',
            message: message,
        })
        console.log({signature})

        const recoveredAddress = await testClient.readContract({
            address: domain.verifyingContract,
            abi: eip712abi,
            functionName: 'getSignerCardSelfService',
            args: [message.operation, message.hashCard, message.hashPin, signature],
        })

        console.log({
            recoveredAddress,
            signaerAddress: account.address
        })

        expect(recoveredAddress).toEqual(account.address)

        /*
       * Validate EIP712 to Backend
       * */
        const cardAccessResponse = await fetch("http://localhost:8080/api/v2/tap2pay/card-access", {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'text/plain',
            },
            body: JSON.stringify({
                signerAddress: account.address,
                hashCard: message.hashCard,
                hashPin: message.hashPin,
                ethSignMessage: signature
            })
        });

        const hsmKey = await cardAccessResponse.text();
        console.log({hsmKey})

    })
});

describe.skip('Gas Estimation', () => {

    test('Access Card', async () => {

        // 0x70997970C51812dc3A010C7d01b50e0d17dc79C8
        const account = privateKeyToAccount('0x59c6995e998f97a5a0044966f0945389dc9e86dae88c7a8412f4603b6b78690d')

        const testClient = createTestClient({
            account,
            chain: foundry,
            mode: 'anvil',
            transport: http(),
        })
            .extend(publicActions)
            .extend(walletActions)

        const gas = await testClient.estimateContractGas({
            address: '0x5FbDB2315678afecb367f032d93F642f64180aa3',
            abi: erc20Abi,
            functionName: 'transferFrom',
            account,
            args: ["0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266", "0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC", BigInt(10_000)],
        })

        const gasPrice = await testClient.getGasPrice();

        console.log({
            gasPrice,
            gas,
            fee: gas * gasPrice,
            feeInEther: formatUnits(gas * gasPrice, 18)
        })

    })

})